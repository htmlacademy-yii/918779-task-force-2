    ...
    <div class="left-menu left-menu--edit">
        ...
        <ul class="side-menu-list">
            <li class="side-menu-item <?= $type !== SettingsForm::SECURITY ? 'side-menu-item--active' : ''; ?>">
                <a href = "<?= Url::to(['/user/settings', 'type' => SettingsForm::PROFILE]); ?>"
                class="link link--nav">Мой профиль</a>
            </li>
            <li class="side-menu-item <?= $type === SettingsForm::SECURITY ? 'side-menu-item--active' : ''; ?>">
                <a href = "<?= Url::to(['/user/settings', 'type' => SettingsForm::SECURITY]); ?>"
                class="link link--nav">Безопасность</a>
            </li>
        </ul>
    </div>
    <div class="my-profile-form">
        <?php $form = ActiveForm::begin(['id' => 'edit-profile-form']);?>
            <h3 class="head-main head-regular">
                <?= $type === SettingsForm::SECURITY ? 'Безопасность' : 'Мой профиль'; ?>
            </h3>            
            ...
            <div class="photo-editing">
                ...
                'class' => 'form-group'], 'labelOptions' => ['class' => 'control-label']])->textInput();
                ?>
            <div class="half-wrapper">
                <?php echo $form->field($model, 'email', ['template' => "{label}\n{input}\n{error}", 'options' => [
                'class' => 'form-group'], 'labelOptions' => ['class' => 'control-label'], 'errorOptions' => [
                    'class' => 'help-block',]])->input('email');
                ?>
                <?php echo $form->field($model, 'birthday', ['template' => "{label}\n{input}\n{error}", 'options' => [
                'class' => 'form-group'], 'labelOptions' => ['class' => 'control-label'], 'errorOptions' => [
                'class' => 'help-block',]])->input('date');
                ?>
            </div>
            <div class="half-wrapper">
                <?php echo $form->field($model, 'phone', ['template' => "{label}\n{input}\n{error}", 'options' => [
                'class' => 'form-group'], 'labelOptions' => ['class' => 'control-label'], 'errorOptions' => [
                'class' => 'help-block',]])->input('tel');
                ?>
                <?php echo $form->field($model, 'telegram', ['template' => "{label}\n{input}\n{error}", 'options' => [
                'class' => 'form-group'], 'labelOptions' => ['class' => 'control-label']])->textInput();
                ?>
            </div>
                <?php echo $form->field($model, 'info', ['template' => "{label}\n{input}\n{error}", 'options' => [
            ...
            <div class="form-group">
                <?php echo $form->field($model, 'categories', [
                    'template' => '{input}'])->checkboxList(ArrayHelper::map($categories, 'id', 'title'), [
                        'class' => 'checkbox-profile', 'itemOptions' => [
                            'labelOptions' => [
                            'class' => 'control-label',
                            ],
                        ],
            ...
            <?php endif; ?>
            <?php if ($type === SettingsForm::SECURITY) : ?>
                <?php echo $form->field($model, 'current_password', [
                'template' => "{label}\n{input}\n{error}", 'options' => [
                'class' => 'form-group'], 'labelOptions' => ['class' => 'control-label']])
                ->passwordInput();
                ?>
                ...
                <?php echo $form->field($model, 'contacts', ['options' => [
